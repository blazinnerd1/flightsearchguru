...
class TeaserFlight extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div>
        <img src={`images/locationPhotos/${flight.to_id}.jpg`} />
        <div>{`AUS to ${flight.to_id} $${flight.price} in ${
          country.name
        }`}</div>
      </div>
    );
